    ...
    <section class="container mx-auto p-4">
        <div class="grid grid-flow-row grid-rows-[max-content_max-content_100px] gap-4">
            ...
            <section v-if="accordion === 'spin_off'" class="grid grid-flow-row gap-8">
                    ...
                    <div class="grid grid-flow-row gap-2">Сиквелы: <br>
            ...
            <section v-if="accordion === 'soundTracks'" class="grid gap-4">
                ...
                >
                    ...
                    <label> Аудио файл: <br>
                        <div class="py-2">Текущий файл: <span class="text-emerald-600">{{ elem.file }}</span></div>
                    </label>
                    <div>Заголовок: <br>
                        <span>{{ elem.title }}</span>
                    </div>
                    <button type="button" @click.prevent="deleteSoundTrack(index, elem.id)"
                            class="col-span-2 justify-self-end px-6 border-2 border-red-700 rounded-2xl text-red-700 hover:bg-red-700 hover:text-white"
                    >
                        <i class="icon-delete font-bold text-2xl"></i>Удалить
                    </button>
                </form>
                ...
                <form v-for="(new_sound_track, index) in form.new_sound_tracks"
                      class ="grid grid-cols-2 gap-2 bg-white p-4 justify-start"
                >
                    <h6 class="col-span-2">Новый Саунд Трек:</h6>
                    <label> Выберите файл <br>
                        <input class="py-2" type="file" @input="handleFile(index, $event)" accept="audio/*">
                    </label>
                    <label>Заголовок: <br>
                        <input class="py-2" type="text" v-model="new_sound_track.title">
                    </label>
                    <button type="button" @click.prevent="deleteNewSoundTrack(index)"
                    class="col-span-2 justify-self-end px-6 border-2 border-red-700 rounded-2xl text-red-700 hover:bg-red-700 hover:text-white"
                    >
                        <i class="icon-delete font-bold text-2xl"></i>Удалить
                    </button>
                ...
                <button @click.prevent="addSoundTrack()" type="button"
                        class="bg-green-800 text-white box-border px-6 py-2 mr-6 rounded-md justify-self-start
...
export default {
    ...
    data() {
        return {
            ...
            form: {
                ...
                sound_tracks: this.movie.soundtracks,
                new_sound_tracks: [],
                s_tracks_deleteIds: [],
            },
            previews: {
    ...
    methods: {
        ...
        handleFile(index, event) {
            this.form.new_sound_tracks[index].file = event.target.files[0];
        },
        ...
        addSoundTrack() {
            this.form.new_sound_tracks.push(
                {
                    file: '',
                    title: '',
                }
        ...
        deleteSoundTrack(elemIndex, elemId) {
            this.form.s_tracks_deleteIds.push(elemId)
            this.form.sound_tracks = this.form.sound_tracks.filter((elem, index) => {
                return index != elemIndex
            })
        },
        deleteNewSoundTrack(elemIndex) {
            this.form.new_sound_tracks = this.form.new_sound_tracks.filter((elem, index) => {
                return index != elemIndex
            })
        },
        ...
        updateMovie() {
            ...
            this.form.new_sound_tracks = this.form.new_sound_tracks.filter((elem) => {
                if (elem.file != '' && elem.title != '') {
                    return elem
                }
            })
            router.post(`/admin/movies/${this.form.id}`, {
                    ...
                    s_tracks_deleteIds: this.form.s_tracks_deleteIds,
                    new_s_tracks: this.form.new_sound_tracks
                },
                {forceFormData: true,}
            )
