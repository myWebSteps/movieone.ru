<template>
    ...
    <div v-for="movie in data" :class="config.classes" class="shadow-md bg-white rounded-sm overflow-hidden mb-4">
            ...
            <Link :href="`/movies/${movie.slug}`" class="h-full grid grid-rows-[max-content,_1fr] items-start">
                    ...
                    <img v-lazy="movie.poster" :alt="movie.nameRu + ' постер'" class="w-full" width="267" height="400">

                <article class="p-2 grid gap-1 self-stretch max-w-[200px]">
                    <h2 class="text-gray-900 text-wrap">{{movie.nameRu}}</h2>
                    ...
                    <h5 class="grid grid-flow-col auto-cols-max auto-rows-max self-end">
